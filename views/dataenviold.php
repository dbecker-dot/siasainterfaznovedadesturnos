...
$datvigiloo=new vigilooapp();
   ...
   echo '-------------------CARGA CAMION FIN-------------------</br>';
    
    
   $novedades= $datvigiloo->listarnovedadesccfin();
   foreach ($novedades as $row) :
        $datvigiloo->tiponovedad='CCF';
        $datvigiloo->ptu_numero=$row['turno'];
        $datvigiloo->fechahoranovedad=$row['Cargacamfin'];
        if ($datvigiloo->checknovedadregistrada()==0){
            $datvigiloo->insertarnovedadturnoenbdsiasa();
            echo $row['turno'].' - '.$row['Cargacamfin'].'</br>';
        }
    endforeach;
    
   echo '</br>//////////////////////////CARGA CAMION FIN////////////////</br>';
   

   echo '-------------------ADMINISTRACION INI-------------------</br>';
    
    
   $novedades= $datvigiloo->listarnovedadesadminicio();
   foreach ($novedades as $row) :
        $datvigiloo->tiponovedad='ADMI';
        $datvigiloo->ptu_numero=$row['turno'];
        $datvigiloo->fechahoranovedad=$row['Admini'];
        if ($datvigiloo->checknovedadregistrada()==0){
            $datvigiloo->insertarnovedadturnoenbdsiasa();
            echo $row['turno'].' - '.$row['Admini'].'</br>';
        }
    endforeach;
    
   echo '</br>//////////////////////////ADMINISTRACION FIN////////////////</br>';

   echo '-------------------ADMINISTRACION FIN-------------------</br>';
    
    
   $novedades= $datvigiloo->listarnovedadesadmfin();
   foreach ($novedades as $row) :
        $datvigiloo->tiponovedad='ADMF';
        $datvigiloo->ptu_numero=$row['turno'];
        $datvigiloo->fechahoranovedad=$row['Admfin'];
        if ($datvigiloo->checknovedadregistrada()==0){
            $datvigiloo->insertarnovedadturnoenbdsiasa();
            echo $row['turno'].' - '.$row['Admfin'].'</br>';
        }
    endforeach;
