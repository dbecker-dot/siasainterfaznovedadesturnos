...
class vigilooapp {
    ...
    public function cantnovedadesnoinformadas(){
        $cant=0;
        try {
            $this->sql="SELECT cant FROM vista_novedadesvigiloonoinformadas";
            $this->conexsiasa= $this->db->conectar();
            $this->stmt = sqlsrv_query($this->conexsiasa, $this->sql );
            if( $this->stmt === false) {
                $this->db->cierraconexion();
    ...
    public function listarnovedadesparareenviar(){
        try {
            $datos=[];
            $datosaux=[];
            $this->sql="SELECT * FROM vista_ctrolinfvigiloo where estado='ER' or estado='RE' ";
            $this->conexsiasa= $this->db->conectar();
            $this->stmt = sqlsrv_query($this->conexsiasa, $this->sql );
            if( $this->stmt === false) {
                $this->db->cierraconexion();
    ...
    public function ultimoturnocreacionpapeles(){
        try {
            $datos=[];
            $datosaux=[];
            $this->feultimanovedad='2021/06/24 07:02';
            $this->sql="SELECT MAX(fechahoranovedad) as fechahoranovedad FROM novedadturno where tipo='CP'";
            $this->conexsiasa= $this->db->conectar();
            $this->stmt = sqlsrv_query($this->conexsiasa, $this->sql );
            if( $this->stmt === false) {
                $this->db->cierraconexion();
            ...
            while( $row = sqlsrv_fetch_array( $this->stmt, SQLSRV_FETCH_NUMERIC) ){
                IF ($row[0] == null){
                    $this->feultimanovedad='2021/12/03 12:02';
                } else {
    ...
    public function listarnovedadescreacionpapeles(){
        try {
            $datos=[];
            $datosaux=[];
            $this->ultimoturnocreacionpapeles();
            $this->sql="SELECT NUMERO,[Creacion de Papeles (IN-OUT) Fh Inicio] AS Creacionpapeles FROM DTS_PLANIFICACIONES_TURNOS  WHERE   [Creacion de Papeles (IN-OUT) Fh Inicio] > '$this->feultimanovedad' order by [Creacion de Papeles (IN-OUT) Fh Inicio]";
            $this->conexrds= $this->dbrds->conectar();
            $this->stmt = sqlsrv_query($this->conexrds, $this->sql );
            if( $this->stmt === false) {
                $this->dbrds->cierraconexion();
            ...
            while( $row = sqlsrv_fetch_array( $this->stmt, SQLSRV_FETCH_NUMERIC) ){
                $datos[$i]=['turno'=>$row[0],
                    'Creacionpapeles'=>$row[1]->Format('Y-m-d H:i')
                ];
...
public function ultimoturnoccfin(){
    ...
    public function listarnovedadesarribos(){
        try {
            ...
            $this->sql="SELECT NUMERO,[Arribo Camion Fh Inicio] AS ARRIBO FROM DTS_PLANIFICACIONES_TURNOS  WHERE   [Arribo Camion Fh Inicio] > '$this->feultimanovedad' order by [Arribo Camion Fh Inicio]";
            $this->conexrds= $this->dbrds->conectar();
            $this->stmt = sqlsrv_query($this->conexrds, $this->sql );
            if( $this->stmt === false) {
                $this->dbrds->cierraconexion();
    ...
    public function ultimoturnoingresocamion(){
        try {
            $datos=[];
            $datosaux=[];
            $this->feultimanovedad='2021/06/24 07:02';
            $this->sql="SELECT MAX(fechahoranovedad) as fechahoranovedad FROM novedadturno where tipo='IC'";
            $this->conexsiasa= $this->db->conectar();
            $this->stmt = sqlsrv_query($this->conexsiasa, $this->sql );
            if( $this->stmt === false) {
                $this->db->cierraconexion();
    ...
    public function listarnovedadesingresocamion(){
        try {
            $datos=[];
            $datosaux=[];
            $this->ultimoturnoingresocamion();
            $this->sql="SELECT NUMERO,[Ingreso a Planta Fh Inicio] AS Ingresocamion FROM DTS_PLANIFICACIONES_TURNOS  WHERE   [Ingreso a Planta Fh Inicio] > '$this->feultimanovedad' order by [Ingreso a Planta Fh Inicio]";
            $this->conexrds= $this->dbrds->conectar();
            $this->stmt = sqlsrv_query($this->conexrds, $this->sql );
            if( $this->stmt === false) {
                $this->dbrds->cierraconexion();
            ...
            while( $row = sqlsrv_fetch_array( $this->stmt, SQLSRV_FETCH_NUMERIC) ){
                $datos[$i]=['turno'=>$row[0],
                    'Ingresocamion'=>$row[1]->Format('Y-m-d H:i')
                ];
    ...
    public function listarnovedadescargacamion(){
        try {
            ...
            $this->sql="SELECT NUMERO,[Operacion Carga y Descarga Fh Inicio] AS CARGA FROM DTS_PLANIFICACIONES_TURNOS  WHERE   [Operacion Carga y Descarga Fh Inicio] > '$this->feultimanovedad' order by [Operacion Carga y Descarga Fh Inicio]";
            $this->conexrds= $this->dbrds->conectar();
            $this->stmt = sqlsrv_query($this->conexrds, $this->sql );
            if( $this->stmt === false) {
                $this->dbrds->cierraconexion();
...
public function ultimoturnoadmini(){
    ...
    public function listarnovedadessalidacamion(){
        try {
            ...
            $this->sql="SELECT NUMERO,[Salida de Planta Fh Inicio] AS SALIDA FROM DTS_PLANIFICACIONES_TURNOS  WHERE   [Salida de Planta Fh Inicio] > '$this->feultimanovedad' order by [Salida de Planta Fh Inicio] ";
            $this->conexrds= $this->dbrds->conectar();
            $this->stmt = sqlsrv_query($this->conexrds, $this->sql );
            if( $this->stmt === false) {
                $this->dbrds->cierraconexion();
    ...
    public function modificarestadonovedadturnoenbdsiasa(){
        try
        {
            $this->sql="INSERT INTO ctrolinfvigiloo(idnovedadturno,resultadows,estado) VALUES($this->idnovedadturno,'$this->resultadows','$this->estadoenvio')";
            $this->conexsiasa= $this->db->conectar();
            $this->stmt = sqlsrv_query($this->conexsiasa, $this->sql );
            if( $this->stmt === false) {
                $this->db->cierraconexion();
