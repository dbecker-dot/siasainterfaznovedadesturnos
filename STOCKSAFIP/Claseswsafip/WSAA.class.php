...
class WSAA extends SoapClient {
	...
	private function signTRA() {
		$STATUS = openssl_pkcs7_sign(realpath('.') . '/' . "TRA.xml", realpath('.') . '/' . "TRA.tmp", "file://" . realpath('.') . '/Claseswsafip/Certificados/' . CERT,
    			array("file://" . realpath('.') . '/Claseswsafip/Certificados/' . PRIV_KEY, PASSPHRASE),
    			array(),
    			!PKCS7_DETACHED);
